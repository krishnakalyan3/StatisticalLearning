...
```
Now we will load package `e1071` which contains the `svm` function we will use. We then compute the fit. Notice that we have to specify a `cost` parameter, which is a tuning parameter.
...
The first thing we will do is make a grid values for X11 and X2. We will write a function to do that, in case we want to reuse it. It uses the handy function `expand.grid`, and produces the cooridinates of `n*n` points on a lattice covering the domain of `x`. Having made the lattice, we make a prediction at each point on the lattice. We the plot the lattice, color-coded accordaing to the classification. Now we can see the decision boundary.
